...
These are the major steps for installing the EDA deployment. This applies to both air-gapped installations and Internet-based installations. Some steps will differ depending on the type of installation, this will be clearly called out highlighting both options.
...
2. [Generating the Talos machine configurations](setting-up-the-eda-virtual-machine-nodes.md#generating-the-talos-machine-configurations)  
    Using the Nokia EDA ADM tool and the configuration file, this task generates specific Talos machine configuration files for each Talos VM.
...
5. [Installing the Nokia EDA application](installing-the-eda-application.md)  
    Using the Nokia EDA Installation playground, this step installs EDA on the Kubernetes environment in the Nokia EDA nodes.
