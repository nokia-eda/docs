...
```bash
edaadm bootstrap-k8s -c eda-6-node.yaml #(1)!
```
...
```bash
edaadm get-kubeconfig -c eda-6-node.yaml
```
...
```bash
# check the kubeconfig file path for your deployment
export KUBECONFIG=eda-compute-cluster/kubeconfig.yaml
```
