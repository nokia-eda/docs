# Setting up the Nokia EDA virtual machine nodes

This section describes how to prepare the configuration file, generate the configuration files, and deploy the Talos virtual machines.
...
IPv6 and dual-stack are supported from Nokia EDA 25.8.2 onwards.
////////
///////
...
A common use case is to specify the interface used by Flannel when the default route is not set up or the CNI needs to be bound to a specific interface due to security or operational reasons.
////////
///////
...
////////
//////// html | td
An optional set of PEM-formatted certificates that need to be trusted; this setting is used to trust external services.
////////
///////
...
////////
//////// html | td
Required only for air-gapped environments. You can set the following settings:
...
The `edaadm generate` command creates the Talos configuration file (`talosconfig.yaml`), among other files. The Talos configuration file contains the client certificate for the Talos API. The lifetime of this certificate is 1 year by default. If you need to change the lifetime of the client's certificate, run the `edaadm generate` command with the `--client-cert-duration` flag:
...
This section provides the procedures for deploying a Nokia EDA node as a virtual machine on KVM or VMware vSphere.

### Creating the VM on bridged networks on KVM

Complete the following steps to deploy a Nokia EDA node as a virtual machine on KVM. These steps are executed directly on the Red Hat Enterprise Linux or Rocky Linux hypervisor. The steps below assume the deployment of the `eda-node01` virtual machine as per the above configuration file. Ensure that you use the correct machine configuration file generated by the `edaadm` tool.

/// admonition | Note
    type: subtle-note
This procedure expects two networks to be available on the KVM hypervisors. The OAM network is referred to as `br0` and the fabric management network is referred to as `br1`. Both of these networks are standard Linux bridge networks. If you use only one interface, adapt the `virt-install` command in step 4 to use only the `br0` network.
///
...
3. Create the cloud-init ISO file for the Nokia EDA nodes.
    ...
    * `user-data` file containing the Talos machine configuration file for the corresponding Nokia EDA VM.
...
Complete the following steps to deploy a Nokia EDA node as a virtual machine on VMware vSphere. The steps below assume the deployment of the `eda-node01` virtual machine as per the above configuration file. Ensure that you are using the correct machine configuration file generated by the `edaadm` tool.
...
/// admonition | Note
    type: subtle-note
This procedure expects two networks (port groups) to be available on the ESXi hypervisors. The OAM network is referred to as OAM and the fabric management network is referred to as FABRIC. Both of these networks can be standard port groups or distributed port groups. If you only use one network, you do not need to create a second interface on the VM.
///
...
3. Create a base64-encoded string from the Talos machine configuration for the node.
...
4. Deploy the OVA image using the OVF Tool.
    ...
    /// admonition | Note
        type: subtle-note
    If you prefer using the VMware vCenter UI to create the virtual machines, use the regular method of deploying an OVA/OVF template. In this process, in the Customize template step, when you are prompted to provide the Inline Talos config, you must provide the base64-encoded data from the Talos machine configuration for the node. Use the string returned by the `base64 -i eda-node01.yaml` command and paste it into the field in the UI.
    ///
...
5. In vCenter, edit the VM settings.
    ...
    * Increase the memory to 64 GB.
    * Set the NIC type to E1000 or disable hardware-assisted offloading as recommended in the [Talos documentation](https://docs.siderolabs.com/talos/-{{ talos_version }}-/platform-specific-installations/virtualized-platforms/vmware#validate-the-configuration-files).
    * Increase the main disk size to 100 GB. On boot, Talos automatically extends the file system.
    * Optionally, if this VM is a storage node, add a new disk with a size of 300 GB.
    * Optionally, add a second network interface and connect it to the FABRIC port group.
