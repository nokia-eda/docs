...
: The Apps API is where every App installed into EDA, exposes its custom resources. That includes all the default installed Apps.
    ...
    !!! Note "EDA API vs Kubernetes API & `kubectl`"
...
!!! Note "Do not change or manipulate any of the HTTPProxy API settings as this can break your EDA deployment."
...
!!! Note "Some of the hardcoded settings might change in the future"
...
```bash
curl -s http://###EDA-HOST:PORT###/core/apiproxy/v1/keycloak/realms/eda/protocol/openid-connect/token \
...
??? Note "Example output parsed using `jq`"
    ```bash
    $ curl -s http://eda.domain.tld:9200/core/apiproxy/v1/keycloak/realms/eda/protocol/openid-connect/token \
      -H 'Content-Type: application/x-www-form-urlencoded' \
      --data-urlencode 'client_id=eda' \
      --data-urlencode 'grant_type=password' \
      --data-urlencode 'scope=openid' \
