...
* `Intents Run` - Configuration intent scripts executed during the transaction.
...
1. Transaction Topology is not available for transaction results with more than 10,000 resources.
///

### Detail Level

For all transactions committed to Git, EDA can always display the input resources and their diffs.  
Data not in Git (e.g. failed transaction, dry-run transactions, output resources, node configuration diffs, etc.) are stored in-memory for a limited time.

There are three possible detail levels for transaction results:

* `Detailed`
    * Includes full transaction results
    * All transactions created via EDA UI use this detail level
* `Standard`
    * Does not include the changed resources list, changed resource diffs, and node configuration diffs
    * This is the default detail level for transactions created via EDA API and Kubernetes. It is intended for optimized performance when processing frequent transactions from 'machine interfaces'.
* `Basic`
    * Includes only data available in Git

EDA uses the following rules for removing detailed and standard transaction results:

* Keep details for a guaranteed 25 transactions per user[^4]
* Keep a maximum 10,000 resource diffs per user — details from the oldest transactions will be purged if this limit is exceeded
* Committed transactions are reduced to the basic detail level
* Failed and dry-run transactions are purged from the transaction log
...
The EDA UI offers an 'Advanced' toggle for transaction results. When toggled off, internal EDA resources are hidden from the results.
...
* `Revert` sets all the input resources from a specific transaction back to the previous commit[^5]
...
Don't worry, EDA detects the deviation and commits it to the transaction log.
...
[^5]: If a more recent transaction made changes to any of the input resources, revert will fail. This prevents 'undoing' changes that are not part of the selected transaction.