...
Transactions form the foundation of Nokia Event-Driven Automation (EDA) configuration deployment and revision control. They apply configuration changes atomically, network-wide.
...
Changes within a transaction must succeed or fail together. If the change is not deployable on any of the target nodes, the whole transaction fails and the changes are reverted from all nodes.

### Pipelines and transactions

<!-- EDA 4535 Pipline results in UI -->

A transaction may trigger one or more pipelines when its changed resources match a `PipelineDefinition`.

Pipelines run automated pre-commit and post-commit work around the transaction commit. Pre-commit pipeline jobs run first; the transaction is committed only if those jobs succeed, and post-commit jobs run afterward.

Pipelines are optional for a transaction: if no pipeline definition matches the resources in the transaction, the transaction proceeds normally.

For more information about pipelines, see [Pipelines](pipelines.md).

### Commits

When you commit, the complete transaction succeeds or fails as a unit. If any part of the transaction fails, the whole transaction is rolled back.
...
/// admonition | Note
    type: subtle-note
Output Resources often include internal Nokia EDA resources that are not user-facing. For example, a Nokia EDA configuration intent may create a "state" resource that initializes the process for reporting operational status.
///
...
- When more than 10,000 resource diffs are in memory, reduce details from the oldest transactions until there are fewer than 10,000 resource diffs.

### Revert

The **Revert** action reverses the changes from a specific transaction. On revert, Nokia EDA creates a new additive commit that sets the input resource changes from a specific transaction back to their previously committed value.

If any change in the revert operation conflicts with the latest resource values, EDA will detect this and fail the transaction. This prevents a revert operation from undoing changes to the resource that were committed more recently than the transaction being reverted.
...
The **Restore** action selects a specific past commit and resets all Nokia EDA resources, apps, and allocations to their state at the time of the specified commit. To do this, it may undo the changes from multiple transactions.

/// admonition | Note
    type: subtle-note
Executing a Restore operation requires Read-Write permission for the transaction restore API endpoint.
///
...
- When you make any change to a resource, Nokia EDA offers choices to:
    ...
    - **Commit without pipeline**: Commit the change immediately, and do not run any matching pipelines that a regular commit would invoke.
    ...
    /// admonition | Note
        type: subtle-note
    **Commit without pipeline** might be warranted if you need to avoid the delays that a pipeline's pre-commit jobs might introduce. This option may bypass validation or other operational safeguards that are defined as part of pipelines.
    ///

    /// admonition | Note
        type: subtle-note
    EDA does not run pipelines during a **dry run**.
    ///

- The basket icon at the top of the Nokia EDA UI indicates how many resource changes are pending. Clicking the icon opens the **Transactions** basket.
...
- Any resource that is in the basket will display a basket icon beside its name in a corresponding resource list. If a resource has been un-selected in the basket in order to exclude it from the transaction, this icon is not displayed beside the resource.
    ...
    -{{image(url="graphics/resource-with-basket.png", title="A resource list showing the basket icon", shadow=true, padding=20, scale=0.3)}}-

- The **Transactions Log** displays a record of past transactions.
...
-{{image(url="graphics/basket-one-item.png", title="The Transactions basket showing one pending resource change", shadow=true, padding=20)}}-
...
<!-- EDA 4535 Pipline results in UI -->
- commit the transaction (with or without invoking any pipelines)
...
You also use the **Transactions** basket to view the **Transactions Log**.

-{{image(url="graphics/basket-with-callouts.png", title="The Transactions basket", shadow=true, padding=20, scale=0.5)}}-
...
|8|Transaction options:<ul><li>**Dry Run**: perform a dry run to evaluate the effects of the transaction before committing.</li><li>**Commit**: proceed with updating the selected resources as part of a transaction.</li><li>**Commit without pipeline**: proceed with updating the selected resources as part of a transaction, but do not run any matching configured pipelines and their pre- and post-commit jobs.</li><li>**Create merge request**: create a merge request consisting of the selected resource configuration changes, and empty the basket. The merge request can be performed as a transaction later on the **Merge Requests** page.</li></ul>|

/// admonition | Note
    type: subtle-note
When your transaction is in progress, and while pipelines are running, a blue dot pulses beside the basket icon. If the transaction is successful, the dot disappears. If the in-progress transaction or pipeline fails while the basket is closed, the blue dot becomes a red dot to notify the user.  A post-commit pipeline failure may still leave the transaction committed.
///
...
The configuration change for this resource is added as an item in the basket. The change is not applied until you commit.
...
Closing the Transactions basket dismisses the form, but retains information about the most recent dry run. If changes occur to the basket contents, any dry run information is cleared and the form is restored to the normal list view.

#### Resolving conflicts

Each time the basket is opened, EDA checks if resource changes in the basket conflict with the latest resource values. This may occur if another user edits a resource after you added your change to the basket. In this case, a **Resolve conflicts** button is displayed. Clicking this button opens a "three-way merge" tool. This is the same tool used when resolving conflicts in merge requests, for see [Resolving conflicts](merge-requests.md#resolving-conflicts).

-{{image(url="graphics/basket-conflict.png", title="The **Transactions** basket showing a conflict and the **Resolve conflicts** button", shadow=true, padding=20, scale=0.5)}}-
...
The following information displays in the transactions log:
...
- **Add to Basket**: adds the transaction's input resources to the [Transactions basket](#the-transactions-basket). This allows you to retry the transaction (or a modified version of a previous transaction).
...
While the **Transactions Log** is a list of past transactions, the **Transaction Summary** page shows detailed information about a single transaction.

-{{image(url="graphics/transaction-summary.png", title="The Transaction summary page", shadow=true, padding=20)}}-
...
<!-- EDA 4535 Pipline results in UI -->
When pipeline results are relevant, the summary also displays a **Navigate to pipeline** icon.

-{{image(url="graphics/summary-pipeline.png", title="The summary showing the pipeline icon", shadow=true, padding=20)}}-

Clicking this icon takes you to the pipeline runner summary. If a transaction failed due to a pipeline error, use this view to troubleshoot the issue.
...
From the breadcrumb at the top of the Transaction Summary page you can select **Diffs** to view the **Transaction diffs** view. This view shows details of the specific configuration changes that were part of this transaction.

-{{image(url="graphics/transaction-diffs.png", title="The Transaction Diffs view", shadow=true, padding=20)}}-
...
Links in the **Transaction Topology** page represent resource relationships established by the intent runs where:
