...
Using the same `demo` repository from the [Installation section](../getting-started/install.md), we can use the same `make` targets to perform the installation steps (if haven't done before).
...
EDA UI exposed inside the cluster via the `eda-api` service of type LoadBalancer and its `api-server` port (9200):

```bash title="service configuration"
kubectl get service eda-api -o yaml | yq e '.spec.ports[0]' - #(1)!
```
...
If you're using Nginx Ingress controller, you drew the lucky straw, since `make eda-bootstrap` installs the Ingress resource[^1] for you and you can access the EDA UI by navigating to the Nginx Ingress controller URL.

#### Gateway API

If you're riding the Gateway API wave, you can create a `Gateway` resource to define your traffic gateway and then add an `HTTPRoute` resource to expose the EDA UI service.

It may very well be that you already have a `Gateway` resource defined in your cluster. But if not, let's create one:
...
Now, let's create the `HTTPRoute` resource to expose the EDA UI service and connect it to the `Gateway` resource:

/// tab | YAML Resource

```yaml
--8<-- "docs/user-guide/ingress/httproute.yml"
```
...
With Traefik ingress controller deployed in your cluster with the default `web` endpoint, you can create the Traefik `IngressRoute` resource to expose the EDA UI over the `web`'s endpoint over the http port:

/// tab | YAML Resource

```yaml
--8<-- "docs/user-guide/ingress/traefik-ingress.yml"
```
...
With this `IngressRoute` resource in place, you can access the EDA UI by navigating to the `http://<traefik-ingress-service>/` URL.
...
[^1]: See the resource with `kubectl get eda-api-ingress -o yaml`
