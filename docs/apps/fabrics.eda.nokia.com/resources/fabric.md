...
/// admonition | Why one ASN per leaf switch?
    type: question
...
/// admonition | Why one ASN for all spine switches?
    type: question
...
/// admonition | Don't forget this property!
    type: warning
...
- System IP addresses for the [`SystemInterface`](-{{ ref_app_doc('routing', 'systeminterface') }}-) resource on each node in the `Fabric`, drawn from an `IPAllocationPool`
- Point-to-point IP subnets for the [`ISL`](#inter-switch-links), drawn from a `SubnetAllocationPool`
- Autonomous System Numbers (ASNs) for the [`DefaultRouter`](-{{ ref_app_doc('routing', 'defaultrouter') }}-) resource on each node in the `Fabric`, drawn from an `IndexAllocationPool`
...
The **underlay** of a datacenter refers to the exchange of reachability information that enables the [overlay](-{{ ref_app_doc('routing', 'index.md') }}-#overlay-routing) routes to be exchanged. The `Fabric` resource uses the **underlay** protocol for the exchange of system IP addresses, which will be used to establish the MP-BGP session for the exchange of EVPN routes. Currently the following protocols are supported for the exchange of system IP addresses:

- eBGP (iBGP not supported)
- OSPFv2
- OSPFv3

/// admonition | Is exchange of EVPN routes via the underlay supported?
    type: question
...
/// admonition | MTU considerations for OSPF interoperability
    type: warning
...
If the MTU is not set using the [`DefaultMTU`](-{{ ref_app_doc('siteinfo', 'defaultmtu') }}-) resource, it is important to set the `ipMTU` property of the `interSwitchLinks` container in the `Fabric` resource. An MTU value of `8922` works for most interop scenarios.
///
...
/// admonition | Which protocol to choose?
    type: question
...
If not explicitly specified, the Fabric will **automatically generate** the required [Policy](-{{ ref_app_doc('routingpolicies', 'policy') }}-) resources. These policies are used in the BGP peering sessions to ensure IP reachability across the fabric.

If [`routing policies`](-{{ ref_app_doc('routingpolicies', 'policy') }}-) are defined independently of the `Fabric` through the `importPolicies` or `exportPolicies` properties, they will be used instead.

## Route leaking

Route leaking is used to establish connectivity between the [`DefaultRouter`](-{{ ref_app_doc('routing', 'defaultrouter') }}-) and virtual networking services. For example, it may be done on the [borderleaf nodes](#borderleaf-nodes) to expose an isolated in-band management network to the WAN.

Route leaking relies on [`routing policies`](-{{ ref_app_doc('routingpolicies', 'policy') }}-) and can be specified globally in the `Fabric` resource or overridden under each [node role](#fabric-nodes) container.
...
/// admonition | A note on inter-fabric Links
    type: warning
...
Quality of Service policies can optionally be specified in the `interSwitchLinks` container of the `Fabric` resource. An [`IngressPolicy`](-{{ ref_app_doc('qos', 'ingresspolicy') }}-) is used to assign priorities to incoming traffic, and optionally to rate-limit traffic with a particular priority.

### [`EgressPolicy`](-{{ ref_app_doc('qos', 'egresspolicy') }}-)

Quality of Service policies can optionally be specified in the `interSwitchLinks` container of the `Fabric` resource. An [`EgressPolicy`](-{{ ref_app_doc('qos', 'egresspolicy') }}-) is used to assign packets to [`Queues`](-{{ ref_app_doc('qos', 'queue') }}-) depending on their priority and to modify the priority bits in the headers of outgoing traffic.
