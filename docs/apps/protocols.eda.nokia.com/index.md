...
To facilitate distributed [routing and switching services](-{{ ref_app_doc('services', 'index.md') }}-), network elements need to exchange forwarding and routing information. The Protocols application enables users to create and manage various routing protocols and contains resources that are split between the [Overlay Routing](-{{ ref_app_doc('routing', 'index.md') }}-#overlay-routing) and [Default Routing](-{{ ref_app_doc('routing', 'index.md') }}-#underlay-routing) categories.
