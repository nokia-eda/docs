...
A popular example of resource composition in EDA is the [Fabric resource](../apps/fabrics.eda.nokia.com/resources/fabric.md) that we mentioned earlier. The Fabric is a high-level abstraction that represents a Clos-based EVPN VXLAN fabric of an arbitrary size. When a user creates a Fabric resource, EDA configures on the target devices multiple entities, such as:
...
[fabric-app-docs]: ../apps/fabrics.eda.nokia.com/index.md
